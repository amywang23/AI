...
#gets the num of conflicts for each queen
def getConflictsforQ(state):
    queenConflicts = [0] * 80
    for row in range(n):
        col = state[row]
...
#finding the queen with max conflicts
def findNextQueen(state):
    queenConflicts = getConflictsforQ(state)
    #printboard(state)
...
#where to move the queen so there is the least conflicts
def getNextMove(state, row):
    minlist = []   #list with all col with min val
    tlist = []    #list with conflict val for all col
    oldcol = state[row]
    oldt = conflicts()
...
def repair(state):
    ...
    while count < 100000:
        ...
        print("State: ", state, ", # of Conflicts: ", conflicts())

    return None

#finds the total number of conflicts for the current board state
def conflicts():
