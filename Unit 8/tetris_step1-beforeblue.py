...
for i in range(len(pieces)):   #go through all pieces and orientations
    ...
    for j in range(drop_col_end+1): #drop from left to right cols
        newboard = iboard
        # if prows == 1 or (prows==2 and pcols==2):      #only one row I0 or O
        #     minh = 20
        #     minc = -1
        #     for k in range(pcols):
        #         if col_height[j+k] < minh:
        #             minh = col_height[j+k]
        #             minc = j+k
        #     print('j', j, "minh", minh, "minc", minc)
        #     if (minh == 0 or (minh==1 and prows==2 and pcols==2)):
        #         print("GAME OVER")
        #     else:
        #         for ii in range(1,prows+1):
        #             for k in range(pcols):
        #                 newboard = replace_cell(newboard, minh-ii, j+k, "#") 

        # if pcols == 1:  #only one col I1
        #     minh = 20
        ...
        #if pcols == 2 or pcols == 3:  #two cols
        if pcols>0:
