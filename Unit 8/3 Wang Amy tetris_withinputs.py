...
def play_game(strategy, printout):
    ###print("\n^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
...
def fitness_function(strategy):
    ...
    #print("Evaluating strategy number", fitcount, "-->", average)
    return  average
...
def calculate_score():
    global generation_count, fitcount, population_score, old_population_score
...
def runit(start_gen):
    ...
    maxstrategry = []
    sorted_score = []
    generation_count = start_gen
    if start_gen == -1:
        generation_count = 0
        population_score = defaultdict(list)
        print("generation", generation_count, "starting to create generation")
        population = generate_first_population()
        #print(population)
        population_score = calculate_score()
        

    for j in range (start_gen, GENERATION_NUMBER):
        ...
        print("Best strategy so far:", maxstrategry, "with score", maxscore)
        # print("overall highest score", all_max_score, all_max_strategy)

        action = input("\n(P)lay a game with current best strategy, (S)ave current process, or (C)ontinue?  ")
        action = action.upper()

        if (action == "P"):
            print("play a game")
            play_game(maxstrategry, True) 
            exit(0)
        if (action == "S"):
            filename = input("\nWhat file name?  ")
            storeData(filename)
        ...
        if (action == "C"):
            ...
            print("generation", generation_count, "starting to create generation size:", len(next_generation))
            population = next_generation
            population_score = calculate_score()
            
            #end = perf_counter()
            #print("#",j,end-start,"-----------------------------------------\n")
        if action != "C" and action != "P" and action != "S":
            print("Not a valid action, exit")
            exit(0)     #not a valid action
    print("maxscore", maxscore, maxstrategry)
   
    return maxscore

def storeData(filename):
    global population_score, generation_count
    # initializing data to be stored in db
    generation_number_item = {'generation_number' : generation_count}
    population_score_item = {'population_score':population_score}
     
    # database
    db = {}
    db['generation_number_item'] = generation_number_item
    db['population_score_item'] = population_score_item
...
def loadData(filename):
    ...
    # for keys in db:
    #     print(keys, '=>', db[keys])
    generation_count = db['generation_number_item']['generation_number']
    population_score = db['population_score_item']['population_score']
    dbfile.close()


NL = input("(N)ew process, or (L)oad saved process?  ")
NL = NL.upper()
if (NL=="N"):
    runit(-1)
if (NL=="L"):
    ...
    loadData(filename)
    print("generation coount", generation_count)
    runit(generation_count)
