...
CO_WELL_DEPTH = -0.4
CO_HOLES_NUMBER = -0.9
CO_LINES_REMOVED = 0.9
CO_NEAR_WALL = 0.2
CO_EDGE_NUMBER = 0.2
CO_ROW_PLACEMENT = 0.5
CO_EMPTY_SPACES = 0.1
...
def play_game(strategy):
    global lines_cleared, picked_strategy
    board = make_new_board()
    points = 0
    gameover = False
    #strategy = generate_strategy() #picked_strategy
    while not gameover:
...
def runit():
    ...
    for j in range (GENERATION_NUMBER):
        start = perf_counter()
        population_score = generate_population_score()
        end = perf_counter()
        print("enter", j, "time", end-start)
        ...
        if (maxscore < sorted_score[0][1]):
            ...
            print("generation", j, "updated maxscore to", maxscore, maxstrategry)
        if maxscore > 50000:
            break
        next_generation = []
