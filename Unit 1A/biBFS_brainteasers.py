...
for index, elem in enumerate(line_list):
    goal = find_goal(elem)
    start = perf_counter()
    # result = BFS(int(elem), elem)
    # end = perf_counter()
    # print("Line", index, ":", elem, ",", result[0], "moves found in", end-start, "seconds")
    # start = perf_counter()
    result = biBFS(4, elem, goal)
    end = perf_counter()
    print("Line", index, ":", elem, ",", len(result)-1, "moves found in", end-start, "seconds")
    print()