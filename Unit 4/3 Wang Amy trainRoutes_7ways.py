    ...
    while (len(q1) > 0 and len(q2) > 0):
        ...
        for nb in edges[city2]:
            ...
            if (new_distance < distances_target[neighbor] ):
                ...
                newpath = list(v2[2])
                newpath.append(neighbor)
                heapq.heappush(q2, (distances_target[neighbor], neighbor, newpath))
                visited2.add(neighbor)
                map2[neighbor] = newpath
                if neighbor in visited1:    #found in searching from start
                    list2 = map2[neighbor]
                    if len(list2) > 0:
                        list2.pop()
                        list2.reverse()
                    print("biDijkstra meet from back distance=", distances[neighbor]+distances_target[neighbor])
...
#algorithm from front and back at the same time
