...
def hill_climbing(word, n):
    ...
    while True:
        ...
        if changecount > 20 and scorediff<10:
        #if nword[0:2] == "CS": #or scorediff<10:
            break
        
    return

def generate_population(word):
    global population, population_score, mapping

    population = set()
    while True:
        alp = random_alphbet()
        if alp in population:
            continue
        population.add(alp)
        mapping = alp
        nword = en_decode(word, 2)
...
#"XMTP CGPQR BWEKNJB GQ OTGRB EL BEQX BWEKNJB, G RFGLI. GR GQ BEQX ABSETQB RFGQ QBLRBLSB TQBQ EJJ RBL KMQR SMKKML VMPYQ GL BLDJGQF: 'G FEUB RM AB E DMMY QRTYBLR GL RFER SJEQQ GL RFB PMMK MC RFER RBESFBP."
#
