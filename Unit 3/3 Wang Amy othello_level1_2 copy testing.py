...
#find all the moves that i can make at this point
...
def max_move(board, depth):
    ...
    temp = possible[0]
    maxval = min_step(make_move(board,"x",temp), depth)
    for ind in possible:
        result = min_step(make_move(board,"x",ind), depth)
        if result == 0:
            break
        if result > maxval:
            maxval = result
...
def min_move(board, depth):
    ...
    temp =possible[0]
    minval = max_step(make_move(board,"o",temp), depth)
    for ind in possible:
        result = max_step(make_move(board,"o",ind), depth)
        if result == 0:
            break
        if result < minval:
            minval = result
...
def max_step(board, depth):
    ...
    for next_board in possible_moves(board, "x"):
        results.append(min_step(make_move(board,"x",next_board), depth-1))
    if len(results) == 0:
        return score(board, "x")
    return max(results)
...
def min_step(board, depth):
    ...
    for next_board in possible_moves(board, "o"):
        results.append(max_step(make_move(board, "o", next_board), depth-1))
    if len(results) == 0:
        return score(board, "o")
    print(min(results))
    return min(results)
