...
def find_next_move(board, player, depth):
   # Based on whether player is x or o, start an appropriate version of minimax
    ...
    for move in moves:
        ...
        val = min_max(newBoard,depth-1, opponent,-maxx,maxx)
        scores[move]=val
    if player == "x":
...
def min_max(board, depth, player, alpha, beta):
    opponent = get_opponent(player)
    me = player
    scores = set()
    current_score = 0
    ...
    moves = possible_moves(board, player)
    if len(moves) == 0:
        return min_max(board,depth, opponent, alpha, beta)
    if me == "x":
        for move in moves:
            newBoard = make_move(board, me, move)
            current_score = min_max(newBoard, depth-1, me, alpha, beta)
