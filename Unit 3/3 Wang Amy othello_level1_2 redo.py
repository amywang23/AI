...
def max_move(board, depth):
    ...
    temp = moves[0]
    max = min_step(make_move(board,"x",temp), depth, -maxx)
    for move in moves:
        result = min_step(make_move(board,"x",move), depth, -maxx)
        if result > maxval:
            maxval = result
            temp = max
    return temp

def max_step(board, depth, maxx1):
    ...
    for next_board in possible_moves(board, current_player):
        results.append(min_step(make_move(board, current_player, next_board), depth-1, m1))
    return max(results)

def min_move(board, depth):
    moves = possible_moves(board,"o")
    temp = moves[0]
    max = min_step(make_move(board,"o",temp), depth, -maxx)
    for move in moves:
        result = min_step(make_move(board,"o",move), depth, -maxx)
        if result > max:
            max = result
            temp = move
    return temp

def min_step(board, depth, maxx1):
    ...
    m1 = maxx1
    for next_board in possible_moves(board, current_player):
        results.append(max_step(make_move(board, current_player, next_board), depth-1, m1))
    return min(results)
