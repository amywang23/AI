...
def guess_data(datalist):
    global caselist_nodups, caselist_dups
    ...
    for d in datalist:
        ...
        if "?" in vals:
            ...
            if ','.join(l) in classificationlist:
                caselist_dups.append(d)
                #print("this is dup", d)
                dupcount += 1
            else:   # not a dup
                caselist_nodups.append(d)
        else:   # no ?
            caselist_nodups.append(d)

    #remove all dupscaselist_dups
...
def generate_tree(datalist, parent, depth):
    ...
    if original!=0 and maxset[2] == question_number-1 :
        print("IMPORTANT ZERO GAIN")
...
def find_max_gain(datalist, original):
    ...
    return(maxquestion, maxgain, zerocount)
